...
exports.plugin = {
    ...
    async register(app, options) {
        ...
        const { getHeaders } = require(path.join(path.dirname(process.mainModule.filename), "/../../core-p2p/dist/socket-server/utils/get-headers"));
        const { Peer } = require(path.join(path.dirname(process.mainModule.filename), "/../../core-p2p/dist/peer"));
        const { isWhitelisted } = require(path.join(path.dirname(process.mainModule.filename), "/../../core-p2p/dist/utils/is-whitelisted"));
